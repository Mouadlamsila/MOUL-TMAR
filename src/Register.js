...
export default function Register(){
    const[Register , setRegister] = useState({Nom : "" , Prenom : "" , Numero :"" , Email : "" , Password : "" })
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const [msg ,setmsg]=useState("")
    function register(){
        if(Register.Nom && Register.Password){
            dispatch(
            {
                type:"register",
                payload:{Register},
        ...
        <div className="flex h-screen  bg-[#4b2d1f]  text-center text-[#f7efe6]  w-full justify-center items-center ">
            <div className=" bg-white rounded-lg shadow-lg p-8 w-full max-w-md text-[#4b2d1f] ">
                ...
                <h1 className="text-2xl  font-bold mb-3 text-[#4b2d1f]">إنشاء حساب</h1>
                    ...
                    <input type="tel" className=" text-end p-1 w-full p-2 mb-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#4b2d1f]" placeholder="رقم الهاتف" onChange={(e)=> setRegister({...Register , Numero :Number(e.target.value)})}/><br/>
